<div class="form-group">
    ...
    <div class="col-xs-12 col-sm-12 col-md-9 col-lg-10 col-content">
        <input type="text" class="delete_footer" name="delete_footer" value="" hidden />
        ...
        <p class="delete-image-preview {{isset($show)?'show deleteimages':''}}" onclick="deleteImagePreview(this);"><i class="fa fa-window-close"></i></p>
        </div>
        @isset( $url )
            <hr>
            <input class="form-control validate[custom[url], maxSize[255]]]" data-prompt-position="bottomLeft:0,11" name="{{ $url }}" type="text" placeholder="Enter Destination URL" value="{{$dataurl}}">
        @endisset
        {{ $slot }}
    </div>
</div>
